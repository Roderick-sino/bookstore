...
<div class="divBody">
	...
	<div class="divContent">
	  <div style="margin: 20px;">
		...
		<c:if test="${msg=='您还没有登录，不能访问本资源'}">
		<span style="margin-left: 50px;"><a target="_top" href="<c:url value='/jsps/user/login.jsp'/>">登录</a></span>
		
		</c:if>
		<c:if test="${msg=='恭喜，交易成功！'}">
		<span style="margin-left: 50px;"><a target="_top" href="<c:url value='/OrderServlet?method=comment&oid=${order.oid }'/>">评论</a></span>
		
		</c:if>
		
		<c:if test="${msg=='评论添加成功！'}">
		<span style="margin-right: 50px;"><a target="_top" href="javascript:history.go(-2)">返回</a></span>
	        		</c:if>
